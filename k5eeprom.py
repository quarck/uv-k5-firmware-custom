...
EEPROM_LIMIT = 0x10000  # 16-bit offset field in CMD_051B_t / CMD_051D_t
...
def check_range(offset, length):
    ...
    if offset < 0 or end > EEPROM_LIMIT:
        sys.exit(f"range 0x{offset:X}..0x{end:X} exceeds the protocol's 16-bit "
                 f"limit of 0x{EEPROM_LIMIT:X}")
    ...
    try:
        ...
        if args.cmd == "write":
            ...
            for off, n in chunks(args.offset, len(data)):
                radio.write(off, data[done:done + n])
                done += n
